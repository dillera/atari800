...
class DeviceTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        while True:
            # Read the 8-byte Altirra header
            header_bytes = bytearray()
            logging.debug("DeviceTCPHandler.handle: Attempting to read 8-byte header...")
            while len(header_bytes) < 8:
                try:
                    chunk = self.request.recv(8 - len(header_bytes))
                except ConnectionResetError:
                    print("Connection reset while reading header")
                    return
                if not chunk:
            ...
            # Unpack header: total length (including header), timestamp
            try:
                total_length, timestamp = struct.unpack('<II', header_bytes)
            except struct.error:
                print("Error unpacking Altirra header. Closing connection.")
                return

            logging.debug(f"DeviceTCPHandler.handle: Read header OK: Length={total_length}, Timestamp={timestamp}")

            if self.verbose:
                print(f"Received Header: Total Length={total_length}, Timestamp={timestamp}")

            if total_length < 10: # Must be at least header(8) + event(1) + arg(1)
                print(f"Invalid total_length {total_length} in header. Closing connection.")
                return
            ...
            while len(payload_bytes) < payload_len:
                 # Limit read size to avoid huge allocation if length is corrupted
                bytes_to_read = min(payload_len - len(payload_bytes), 4096) 
                try:
                    chunk = self.request.recv(bytes_to_read)
                except ConnectionResetError:
                     print("Connection reset while reading payload")
                     return
                if not chunk:
            ...
            # Extract event, arg, and data from payload
            ...
            if self.verbose:
                 ...
                 print(f"Received Payload: Event=0x{event:02X}, Arg=0x{arg:02X}, Data Len={len(data)}, Data=[{hex_data}]")

            try:
                command_name, handler = self.handlers[event]
            except AttributeError:
                 # Should not happen if called via AtDevHandler which defines self.handlers
                 print(f"Error: self.handlers dictionary not found in {self.__class__.__name__}. Closing connection.")
                 return
            except KeyError:
                print(f"Unhandled Altirra event 0x{event:02X} - closing connection.")
                return

            if self.verbose:
                # Similar format to old verbose log, but using event/arg
                print(f"Calling Handler: {timestamp:08X} {command_name}(Event=0x{event:02X}, Arg=0x{arg:02X}, DataLen={len(data)})")

            # Call the handler method found in the dictionary
            # Pass event, arg, data, and timestamp
            try:
                handler(event, arg, data, timestamp) 
            except Exception as e:
                print(f"Error executing handler {command_name} for event 0x{event:02X}: {e}")
                # Decide if we should close connection on handler error? Maybe not always.
                # For now, just print the error and continue the loop.
                import traceback
                traceback.print_exc()
