...
#ifdef USE_FUJINET
/* Counter for tracking command frames */
static int command_frame_counter = 0;
#endif
...
int SIO_Initialise(int *argc, char *argv[])
{
	...
	for (i = j = 1; i < *argc; i++) {
		if (strcmp(argv[i], "-d1") == 0) {
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d1");
				return FALSE;
			}
			if (!SIO_Mount(1, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 1: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
			...
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d2");
				return FALSE;
			}
			if (!SIO_Mount(2, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 2: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
			...
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d3");
				return FALSE;
			}
			if (!SIO_Mount(3, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 3: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
			...
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d4");
				return FALSE;
			}
			if (!SIO_Mount(4, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 4: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
		else if (strcmp(argv[i], "-d5") == 0) {
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d5");
				return FALSE;
			}
			if (!SIO_Mount(5, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 5: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
			...
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d6");
				return FALSE;
			}
			if (!SIO_Mount(6, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 6: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
		else if (strcmp(argv[i], "-d7") == 0) {
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d7");
				return FALSE;
			}
			if (!SIO_Mount(7, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 7: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
			...
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d8");
				return FALSE;
			}
			if (!SIO_Mount(8, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 8: %s", argv[i + 1]);
				return FALSE;
			}
			i++;
		}
			...
			if (i + 1 >= *argc) {
				Log_print("Missing filename for -d");
				return FALSE;
			}
			if (!SIO_Mount(1, argv[i + 1], FALSE)) {
				Log_print("Error mounting disk 1: %s", argv[i + 1]);
			...
			ignore_header_writeprotect = TRUE;
		}
		else if (strcmp(argv[i], "-h") == 0) {
			printf("Usage: %s [-d1|-d2|-d3|-d4|-d5|-d6|-d7|-d8|-d] <filename> [-i]\n", argv[0]);
			printf("       %s [-h]\n", argv[0]);
			printf("  -d1|-d2|-d3|-d4|-d5|-d6|-d7|-d8  Mount disk image in drive 1-8\n");
			printf("  -d                            Mount disk image in drive 1\n");
			printf("  -i                            Ignore write protect bit in ATR header\n");
	...
	if (FujiNet_Initialise(NULL)) {
		...
		fflush(stdout);
		FUJINET_DEBUG_LOG("FujiNet: Initialized successfully");
		
		/* Initialize our state machine for SIO operations */
		SIO_State_Init();
		FUJINET_DEBUG_LOG("SIO State Machine: Initialized");
		
		/* Initialize SIO handler module */
		SIO_Handler_Init();
		FUJINET_DEBUG_LOG("SIO Handler: Initialized");
		
		/* Initialize the FujiNet SIO handler for processing commands */
		FujiNet_SIO_Handler_Init();
		FUJINET_DEBUG_LOG("FujiNet SIO Handler: Initialized");
		
		/* Debug information about the FujiNet and disk configuration */
		FUJINET_DEBUG_LOG("***** CURRENT DEVICE CONFIGURATION *****");
		FUJINET_DEBUG_LOG("FujiNet enabled: %d", fujinet_enabled);
		
		/* Log status of disk drives */
		FUJINET_DEBUG_LOG("Disk drives status:");
		for (i = 0; i < 8; i++) {
			FUJINET_DEBUG_LOG("Drive %d (device ID: 0x%02X): %s", 
				i + 1, 0x31 + i, 
				SIO_drive_status[i] == SIO_OFF ? "OFF" : 
				SIO_drive_status[i] == SIO_NO_DISK ? "NO DISK" : 
				SIO_drive_status[i] == SIO_READ_ONLY ? "READ ONLY" : "READ/WRITE");
		}
		...
		fflush(stdout);
		FUJINET_DEBUG_LOG("FujiNet: Initialization failed");
	}
...
static int Command_Frame(UBYTE *data_buffer)
{
    ...
    FUJINET_DEBUG_LOG("SIO: Processing Command Frame via FujiNet: %02X %02X %02X %02X %02X",
    ...
    FUJINET_DEBUG_LOG("SIO_Handler: FujiNet_ProcessCommand returned %d", fujinet_response_byte);
    ...
    if (sio_response == SIO_ACK) {
        FUJINET_DEBUG_LOG("SIO_Handler: FujiNet command ACKed (returned SIO_ACK 0x%02X), proceeding to data phase.", sio_response);
        ...
        FUJINET_DEBUG_LOG("SIO_Handler: FujiNet command NOT ACKed (returned 0x%02X), resetting SIO state.", sio_response);
        TransferStatus = SIO_NoFrame;
 	...
 	FUJINET_DEBUG_LOG("SIO: Command_Frame received Device ID: 0x%02X", data_buffer[0]);
...
void SIO_SwitchCommandFrame(int onoff)
{
	if (onoff) {
		...
		CommandIndex = 0;      /* Reset for new command */
		ExpectedBytes = 5;     /* Standard SIO command frame size */
#ifdef USE_FUJINET
		FUJINET_DEBUG_LOG("SIO_SwitchCommandFrame: Enabled command frame, set ExpectedBytes=5");
#endif
	}
	else {
		TransferStatus = SIO_NoFrame;
#ifdef USE_FUJINET
		FUJINET_DEBUG_LOG("SIO_SwitchCommandFrame: Disabled command frame");
#endif
	}
}
...
{
	UBYTE data_buffer[256];
	int sector;
	int realsize;

	/* Get command info from OS 0x300 page */
	UBYTE sio_devic = MEMORY_dGetByte(0x300); /* Device ID */
	UBYTE sio_unit = MEMORY_dGetByte(0x301); /* Unit number */
	UBYTE cmd = MEMORY_dGetByte(0x302);      /* Command */
	UWORD data_addr = MEMORY_dGetWordAligned(0x304); /* Data buffer address */
	UWORD data_len = MEMORY_dGetWordAligned(0x308);  /* Data length */
	UBYTE sio_aux1 = MEMORY_dGetByte(0x30a); /* Low byte of sector/aux */
	UBYTE sio_aux2 = MEMORY_dGetByte(0x30b); /* High byte of sector/aux */

	FUJINET_DEBUG_LOG("SIO_Handler: Dev=0x%02X Unit=0x%02X Cmd=0x%02X Aux1=0x%02X Aux2=0x%02X Data=0x%04X Len=%d",
					  sio_devic, sio_unit, cmd, sio_aux1, sio_aux2, data_addr, data_len);

#ifdef USE_FUJINET
	/* When FujiNet is enabled, route all SIO commands to the NetSIO hub */
	if (fujinet_enabled) {
		/* Create command frame from memory values */
		UBYTE command_frame[5];
		command_frame[0] = sio_devic;     /* Device ID */
		command_frame[1] = cmd;           /* Command */
		command_frame[2] = sio_aux1;      /* Aux1 */
		command_frame[3] = sio_aux2;      /* Aux2 */
		command_frame[4] = SIO_ChkSum(command_frame, 4); /* Calculate correct checksum */
		
		FUJINET_DEBUG_LOG("SIO_Handler: Routing command to NetSIO hub - Device=0x%02X, Command=0x%02X, Aux1=0x%02X, Aux2=0x%02X",
						 sio_devic, cmd, sio_aux1, sio_aux2);
						 
		/* Process the command through our handler */
		SIO_Command_Frame cmd_frame;
		cmd_frame.device_id = command_frame[0];
		cmd_frame.command = command_frame[1];
		cmd_frame.aux1 = command_frame[2];
		cmd_frame.aux2 = command_frame[3];
		cmd_frame.checksum = command_frame[4];
		
		/* Get data buffer pointer from Atari memory if applicable */
		UBYTE *data_buffer_ptr = NULL;
		if (data_len > 0 && data_addr != 0) {
			data_buffer_ptr = &MEMORY_mem[data_addr];
		}
		
		/* Process the command using our handler */
		SIO_Handler_Process_Command(&cmd_frame, data_buffer_ptr, data_len);
		
		/* Copy CPU registers back to ensure compatibility with existing code */
		MEMORY_dPutByte(0x023a, sio_devic); /* CDEVIC */
		MEMORY_dPutByte(0x023b, cmd);      /* CCOMND */
		MEMORY_dPutByte(0x023c, sio_aux1); /* CAUX1 */
		MEMORY_dPutByte(0x023d, sio_aux2); /* CAUX2 */
		
		FUJINET_DEBUG_LOG("SIO_Handler: Command processing complete");
		return;
	}
#endif

	/* Original SIO Handler code (unchanged) */
	UBYTE unit;
	UBYTE result = 'N';

	unit = sio_devic + sio_unit - 1;

	/* The OS SIO routine copies device ID to CDEVIC, command ID to CCOMND etc. */
	...
	if (sio_devic != 0x60 && unit < 8 && (SIO_drive_status[unit] != SIO_OFF || BINLOAD_start_binloading)) {
		/* ... original disk command handling ... */
	}
	/* Process cassette commands */
	else if (sio_devic == 0x60) {
		/* ... original cassette command handling ... */
	}

	/* Process CPU registers based on result */
	switch (result) {
...
void SIO_PutByte(int byte)
{
...
#ifdef USE_FUJINET
	/* More verbose logging about the current state and device detection */
	FUJINET_DEBUG_LOG("SIO_PutByte: Entry - byte=0x%02X, TransferStatus=%d, CommandIndex=%d, ExpectedBytes=%d, fujinet_enabled=%d",
	                 byte, TransferStatus, CommandIndex, ExpectedBytes, fujinet_enabled);

	/* When FujiNet is enabled, route all SIO traffic to the NetSIO hub */
	if (fujinet_enabled) {
		FUJINET_DEBUG_LOG("SIO_PutByte: FujiNet enabled, routing byte 0x%02X to refactored handler (state=%d)", 
		                  byte, TransferStatus);
		SIO_Handler_Put_Byte(byte);
		return;
	}
	
	/* Log debugging information when using original handler */
	FUJINET_DEBUG_LOG("SIO_PutByte: Using original handler (byte=0x%02X, state=%d) - FujiNet disabled",
                     byte, TransferStatus);
#endif /* USE_FUJINET */
	...
	case SIO_CommandFrame:
		if (CommandIndex < ExpectedBytes) {
			...
			if (CommandIndex >= ExpectedBytes) {
#ifdef USE_FUJINET
				/* Command frame complete, print summary and increment counter regardless of device */
				command_frame_counter++;
				FUJINET_DEBUG_LOG("SIO_PutByte: COMMAND FRAME #%d COMPLETE: Device=0x%02X, Command=0x%02X, Aux1=0x%02X, Aux2=0x%02X, Checksum=0x%02X",
					command_frame_counter, CommandFrame[0], CommandFrame[1], CommandFrame[2], CommandFrame[3], CommandFrame[4]);
#endif
				if ((CommandFrame[0] >= 0x31 && CommandFrame[0] <= 0x38 && (SIO_drive_status[CommandFrame[0]-0x31] != SIO_OFF || BINLOAD_start_binloading))) {
					...
					FUJINET_DEBUG_LOG("SIO_PutByte: Command frame complete, device 0x%02X is valid (local disk status=%d)", 
						CommandFrame[0], CommandFrame[0] >= 0x31 && CommandFrame[0] <= 0x38 ? 
						SIO_drive_status[CommandFrame[0]-0x31] : -1);
#endif
				...
				// Handle dedicated FujiNet device 0x70
					...
					FUJINET_DEBUG_LOG("SIO_PutByte: Command frame complete, device 0x%02X is FujiNet", CommandFrame[0]);
					TransferStatus = SIO_StatusRead;
					POKEY_DELAYED_SERIN_IRQ = SIO_SERIN_INTERVAL + SIO_ACK_INTERVAL;
				}
#endif
...
{
#ifdef USE_FUJINET
	/* When FujiNet is enabled, route all data retrieval through our refactored handler */
	if (fujinet_enabled) {
		FUJINET_DEBUG_LOG("SIO_GetByte: FujiNet enabled, retrieving byte from refactored handler");
		int byte = SIO_Handler_Get_Byte();
		FUJINET_DEBUG_LOG("SIO_GetByte: Retrieved byte 0x%02X from refactored handler", byte);
		return byte;
	}
	
	FUJINET_DEBUG_LOG("SIO_GetByte: Using original handler (state=%d) - FujiNet disabled",
                    TransferStatus);
#endif
	switch (TransferStatus) {
	case SIO_StatusRead:
		return Command_Frame(DataBuffer);    /* Handle now the command */
